...
def mean_confidence_interval(data, confidence=0.95):
    ...
    return m, m-h, m+h
def _agg_mean_confidence(gdf,valcol='val',num_runs=1,sample_n=None,replace=True):
    o=[]
    gdfq=gdf[~gdf[valcol].isna()]
    for nr in range(num_runs):
        gdfnow=gdfq.sample(sample_n,replace=replace) if sample_n and (replace or len(gdfq)>sample_n) else gdfq.sample(frac=1)
        odx={'_run':nr, '_size_grp':len(gdfq), '_size_run':len(gdfnow)}
        odx['y'],odx['ymin'],odx['ymax']=mean_confidence_interval(gdfnow[valcol])
        for vk,vv in dict(gdfnow.mean()).items(): odx[vk]=vv
        o.append(odx)
    return pd.DataFrame(o)

    
def agg_mean_confidence(df,groupby,num_proc=1,**kwargs):#valcol='val',num_runs=1,sample_n=None):
    odf=pmap_groups(
        _agg_mean_confidence,
        df.groupby(groupby),
        kwargs=kwargs,#dict(valcol=valcol),
        num_proc=num_proc
    )
    # average across runs
    odf=odf.groupby(groupby).mean()
    # fix
    odf.loc[odf.ymin.isna(), 'ymin']=odf['y']
    odf.loc[odf.ymax.isna(), 'ymax']=odf['y']
    return odf.sort_values('_size_grp',ascending=False).reset_index()
