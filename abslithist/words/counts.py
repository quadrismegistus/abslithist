...
def count_absconc_path_psg(path, **attrs):
    try:
        with open(path, encoding="utf-8", errors="ignore") as f:
            ld = count_absconc(
                f.read(), sources={"Median"}, periods={"median"}, incl_psg=True, **attrs
            )
        for dx in ld:
            dx["path"] = path
        return ld
...
def _count_absconc_window(
    ...
    for key in count_keys:
        ...
        if not incl_psg:
            cdx[key2] = ", ".join(
                list(sorted(list(sharedwords), key=lambda w: -countd[w]))[:vocab_len]
            )
    cdx["num_total"] = total
    ...
    if incl_psg:
        ...
        for tok in all_tokens:
            ...
            if psg and psg[-1] in parens:
                ...
                psg.append(tok)
        cdx["passage"] = " ".join(psg).strip().replace("\n", "\n<br>")
    return cdx
    ...
    for dx in get_norms_for_counting(sources=sources, periods=periods):
        ...
        for i, tok in enumerate(tokens):
            ...
            if len(recog_tokens) >= window_len:
                cdx = _count_absconc_window(
                    dx,
                    recog_tokens,
                    all_tokens,
                    incl_psg=incl_psg,
                    psg_as_markdown=psg_as_markdown,
...
def get_corpus_metadata(cname):
    # camel case to snake case
    cname2 = "".join(["_" + c.lower() if c.isupper() else c for c in cname]).lstrip("_")
    path = os.path.join(PATH_LLTK_CORPORA, cname2)
    path_meta1 = os.path.join(path, "metadata.xls")
    path_meta2 = os.path.join(path, "metadata.csv")
    if os.path.exists(path_meta1):
        meta_df = pd.read_excel(path_meta1)
    elif os.path.exists(path_meta2):
        meta_df = pd.read_csv(path_meta2)
    else:
        raise FileNotFoundError(f"No metadata found for corpus {cname}")
    meta_df["path_txt"] = meta_df["id"].apply(
        lambda x: os.path.join(PATH_LLTK_CORPORA, cname2, "txt", f"{x}.txt")
    )
    return meta_df
...
def count_absconc_corpus(
    ...
    # import lltk
    # C=lltk.load(cname)
    meta_df = get_corpus_metadata(cname)

    paths_txt = meta_df.path_txt.tolist()
    path2id = dict(zip(meta_df.path_txt, meta_df.id))
    # execute
    data = pmap_iter(
    ...
    if not ofn:
        ofn = (
            f'{COUNT_DIR}/data.absconc.{cname}{".psgs." if incl_psg else "."}v7.csv.gz'
        )
    header = (
        header_absconc
        if not incl_psg
        else [h for h in header_absconc if not h in eg_keys] + ["passage"]
    )
    print(">> writing to:", ofn)
    writegen(ofn, _gen, header=header)
    # save as feather
    if not incl_psg:
        pd.read_csv(ofn).reset_index().to_feather(os.path.splitext(ofn)[0] + ".ft")
...
def count_absconc_psg(
    txt, incl_psg=True, sources=PSG_SOURCES, periods=PSG_PERIODS, **attrs
):
    df = pd.DataFrame(
        count_absconc(
            txt,
